...
def dic_key_max_count(dic):
	...
	for key, count in dic.items():
		if key!=None and (key_max_count==None or count>dic[key_max_count]):
			key_max_count=key
	return key_max_count

#returns a dictionary of 3 lists: 1st with x coordinates, 2nd with y coordinates, 3rd with colors with numeric values
...
def fill_2d_data(data,x_from,x_to,y_from,y_to,group):
	for y in range(y_from,y_to+1):
		for x in range(x_from,x_to+1):
			if data.get((x,y),None)==None:
				data[x,y]=group

#file name should be with the suffix pgm
#img should be in a dictionary format where keys are the coordinates of a pixel and the value is the grayscale value
...
#Reads the csv file into the table and then separates the table into heading, complete data, incomplete data and then produces also the index number for the column that is not complete, i.e. contains a question mark.
