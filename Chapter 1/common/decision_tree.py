...
import random
import imp
common=imp.load_source('common', '/home/david/Documents/data_science_book/programs/common/common.py')
...
def classify_by_tree_main(tree,var_to_index,feature):
	if tree.is_leaf():
		...
		for child in tree.get_children():
			...
			if child.get_val()==feature[var_to_index.get(child.get_var())]:
				return classify_by_tree_main(child,var_to_index,feature)
		return None
...
def constuct_decision_tree(heading,complete_data,enquired_column):
	return construct_general_tree(0,heading,complete_data,enquired_column,len(heading))

#m is the number of the classifying variables that should be at most considered at each node.
...
def split_data_by_col(data,col):
	...
	for data_item in data:
		if data_groups.get(data_item[col])==None:
...
def add_children_to_node(verbose,node,heading,complete_data,available_columns,enquired_column,m):
	if len(available_columns)==0:
		printfv(2,verbose,"We do not have any available variables on which we could split the node further, therefore we add a leaf node to the current branch of the tree. ")
		add_leaf(verbose,node,heading,complete_data,enquired_column)
		return -1

	printfv(2,verbose,"We would like to add children to the node "+node.name()+".\n")

	selected_col=select_col(verbose,heading,complete_data,available_columns,enquired_column,m)
	...
	if (len(data_groups.items())==1):
		printfv(2,verbose,"For the chosen variable "+heading[selected_col]
	...
	if verbose>=2:
		printfv(2,verbose,"Using the variable "+heading[selected_col]
			+" we partition the data in the current node, where each partition of the data will be for one of the new branches from the current node "+node.name()+". "
			+"We have the following partitions:\n")
		for child_group, child_data in data_groups.items():
			printfv(2,verbose,"Partition for "+str(heading[selected_col])+"="
				+str(child_data[0][selected_col])+": "+str(child_data)+"\n")
	...
	for child_group, child_data in data_groups.items():
		...
		printfv(2,verbose,"\nWe add a child node "+child.name()+" to the node "+node.name()+". "
			+"This branch classifies %d feature(s): "+str(child_data)+"\n",len(child_data))
		add_children_to_node(verbose,child,heading,child_data,list(available_columns),enquired_column,m)
		node.add_child(child)
	printfv(2,verbose,"\nNow, we have added all the children nodes for the node "
		+node.name()+".\n")

#Selects an available column/attribute with the highest information gain.
def select_col(verbose,heading,complete_data,available_columns,enquired_column,m):	
	#Consider only a subset of the available columns of size m.
	printfv(2,verbose,"The available variables that we have still left are "
		+str(numbers_to_strings(available_columns,heading))+". ")
	if len(available_columns)<m:
		printfv(2,verbose,"As there are fewer of them than the parameter m=%d, we consider all of them. ",m)
		sample_columns=available_columns
	else:
		sample_columns=random.sample(available_columns,m)
		printfv(2,verbose,"We choose a subset of them of size m to be "
			+str(numbers_to_strings(available_columns,heading))+".")
	...
	printfv(2,verbose,"Out of these variables, the variable with the highest information gain is the variable "
		+heading[selected_col]+". Thus we will branch the node further on this variable. "
		+"We also remove this variable from the list of the available variables for the children of the current node. ")
	return selected_col

#Converts each number in a list numbers into the string in the list strings with that index.
...
#Calculates the information gain when partitioning complete_data according to the attribute at the column col and classifying by the attribute at enquired_column.
def col_information_gain(complete_data,col,enquired_column):
	...
	for _,data_group in data_groups.items():
		information_gain-=(float(len(data_group))/len(complete_data))*entropy(data_group,enquired_column)
...
def entropy(data,enquired_column):
	value_counts={}
	for data_item in data:
		if value_counts.get(data_item[enquired_column])==None:
...
#Attach the children from the decision tree into the anytree tree format.
def attach_children(parent_node, parent_anytree_node):
	for child_node in parent_node.get_children():
		child_anytree_node=Node(child_node.name(),parent=parent_anytree_node)
